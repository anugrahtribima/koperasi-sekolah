                ...
                <tr>
                    ...
                    @role('bendahara')
                        <td>
                            ...
                            <a 
                            {{--  href="{{ route('loans.destroy', $pengajuan->id) }}"  --}}
                                 class="btn btn-sm btn-danger" onclick="event.preventDefault();
                            document.getElementById('tolak-form').submit();">
                                Tolak
                            </a>
                            <form id="tolak-form" 
                            {{--  action="{{ route('loans.destroy', $pengajuan->id) }}"  --}}
                             method="POST" style="display: none;">
                                @csrf
                            </form>

                            <a 
                            {{--  href="{{ route('loans.print', $pengajuan->id)}}"  --}}
                             target="_blank" class="btn btn-sm btn-info">Cetak</a>
                        </td>
                    @endrole
                </tr>
            @empty
